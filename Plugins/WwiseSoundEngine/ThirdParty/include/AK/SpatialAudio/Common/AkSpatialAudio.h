...
struct AkSpatialAudioInitSettings
{
	...
	AkUInt32 uMaxReflectionPaths;					///< [\ref spatial_audio_experimental "Experimental"] Set a global reflection path limit among all sound emitters with early reflections enabled. Potential reflection paths, discovered by raycasting, are first sorted according to a heuristic to determine which paths are the most prominent. 
													///< Afterwards, the full reflection path calculation is performed on only the uMaxReflectionPaths, most prominent paths. Limiting the total number of reflection path calculations can significantly reduce CPU usage. Recommended range: 10-50. 
...
namespace AK
{
	...
	namespace SpatialAudio
	{
		...
		///
		AK_EXTERNAPIFUNC(AKRESULT, SetMaximumReflectionPaths)(
			AkUInt32 in_uMaxReflectionPaths	///< Maximum number of reflection paths. Valid range [0..[
			);
