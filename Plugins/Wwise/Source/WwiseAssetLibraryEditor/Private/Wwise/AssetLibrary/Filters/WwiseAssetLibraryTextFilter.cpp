...
void UWwiseAssetLibraryTextFilter::FilterSoundBank(const WwiseMetadataSoundBank* SoundBank, FTextFilters Filter)
{
	if(PatternMatch(Filter, &*SoundBank, WwiseRefType::SoundBank))
	{
		...
		if(!ExcludedSoundBank.Contains(ShortName) && ! IncludedSoundBank.Contains(ShortName))
		{
			Filter.IsExclusionFilter ? ExcludedSoundBank.Add(ShortName) : IncludedSoundBank.Add(ShortName);
			for (auto Media : SoundBank->Media)
			{
				ShortName = *Media.ShortName;
				Filter.IsExclusionFilter ? ExcludedMedia.Add(ShortName) : IncludedMedia.Add(ShortName);
			}
		}
	}
}
		...
		const FWwiseAssetLibraryInfo& AssetLibraryInfo)
{
	...
	{
		for(auto TextFilterIt = TextFilters.rbegin(); TextFilterIt != TextFilters.rend(); ++TextFilterIt)
		{
			const auto SoundBank = Event.Value.GetSoundBank();
			FilterSoundBank(SoundBank, *TextFilterIt);
		}
	}
		...
		const WwiseAnyRef& Asset) const
{
	...
	if(AssetType == WwiseRefType::Media ? ExcludedMedia.Contains(AssetName) : ExcludedSoundBank.Contains(AssetName))
	{
		return false;
	}
	for(auto TextFilterIt = TextFilters.rbegin(); TextFilterIt != TextFilters.rend(); ++TextFilterIt)
	{
		const FTextFilters Filter = *TextFilterIt;
