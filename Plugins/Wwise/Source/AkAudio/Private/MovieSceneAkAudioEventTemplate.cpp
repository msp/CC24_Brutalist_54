...
struct FMovieSceneAkAudioEventSectionData
{
	            ...
	            IMovieScenePlayer& Player, FAkAudioDevice* AudioDevice)
	{
		...
		float PlayerTime = GetTimeInSeconds(Context);
		...
		case EMovieScenePlayerStatus::Paused:
			{
				...
				PauseTracker(AudioDevice, PlayerTime);
		...
		case EMovieScenePlayerStatus::Playing:
			{
					...
					bSequencerHasLooped || bWasPaused)
				{
					...
					{
						ResumeTracker(AudioDevice, Player.FindBoundObjects(Operand), Context);
					...
					else /* Otherwise play or re-trigger the Wwise event on a dummy object. */
					{
						if (EventTracker->PreviousEventStartTime == -1.0f || bSequencerHasLooped)
						{
							UE_LOG(LogAkAudio, VeryVerbose,
								   TEXT(
									   "Playing event on a dumming object with player at EventTrackerTime: %.4f, PlayerTime: %.4f"
								   ),
								   EventTracker->PreviousPlayingTime, GetTimeInSeconds(Context))
						...
						else if (RetriggerEvent)
						{
							UE_LOG(LogAkAudio, VeryVerbose,
								   TEXT(
									   "Retriggering event on a dumming object with player at EventTrackerTime: %.4f, PlayerTime: %.4f"
								   ),
								   EventTracker->PreviousPlayingTime, GetTimeInSeconds(Context))
	...
	void PauseTracker(FAkAudioDevice* AudioDevice, float InPlayerTime)
	{
		if (EventTracker->bStopAtSectionEnd)
		{
		...
		EventTracker->PreviousEventStartTime = -1.0f;
		EventTracker->PreviousPlayingTime = InPlayerTime;
	}
...
private:
	...
	/** Resume paused events, and seek to re-synchronize. If not paused, Use ObjectBindingPlay/Retrigger instead */
	void ResumeTracker(FAkAudioDevice* AudioDevice, TArrayView<TWeakObjectPtr<>> BoundObjects,
	                   const FMovieSceneContext& Context)
	{
		if (EventTracker.IsValid() && EventShouldPlay(Context))
		{
			...
			for (auto ObjectPtr : BoundObjects)
			{
				...
				if (AudioDevice)
				{
					for (auto PlayingID : WwiseEventTriggering::GetPlayingIds(*EventTracker))
					{
						WwiseEventTriggering::ResumeAllPlayingIDs(AudioDevice, *EventTracker);
						WwiseEventTriggering::SeekOnEvent(Object, AudioDevice, GetProportionalTime(Context),
	...
	void MasterPlay(FAkAudioDevice* AudioDevice, const FMovieSceneContext& Context)
	{
		if (EventTracker.IsValid() && EventShouldPlay(Context))
		{
			...
			AkPlayingID PlayingID = WwiseEventTriggering::PostEventOnDummyObject(
				AudioDevice, *EventTracker, CurrentTime);
