...
namespace WwiseEventTriggering
{
	...
	void PauseAllPlayingIDs(FAkAudioDevice * AudioDevice, FWwiseEventTracker & EventTracker)
	{
		...
		if (AudioDevice)
		{
			...
			{
				AudioDevice->PausePlayingID(PlayingID, 20, AkCurveInterpolation_Linear);
			}
		}
	}

	void ResumeAllPlayingIDs(FAkAudioDevice * AudioDevice, FWwiseEventTracker & EventTracker)
	{
		ensure(AudioDevice != nullptr);
		if (AudioDevice)
		{
			for (auto PlayingID : GetPlayingIds(EventTracker))
			{
				AudioDevice->ResumePlayingID(PlayingID, 20, AkCurveInterpolation_Linear);
			}
		}
	}
	...
	AkPlayingID PostEvent(UObject* Object, FAkAudioDevice* AudioDevice, FWwiseEventTracker& EventTracker)
	{
		ensure(AudioDevice != nullptr);
