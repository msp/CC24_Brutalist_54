...
void UAkSettings::CreateInitialAssetLibrary()
{
	if(!bInitialAssetLibraryCreated && bPackageAsBulkData && AssetLibraries.Num() == 0)
	{
		if (!EnsurePostEngineInit()) return;

		static const FString AssetName{ TEXT("WwiseSharedAssetLibrary") };
		...
		if (auto* LoadedUObject{ Path.TryLoad() })
		{
			...
			{
				UE_LOG(LogAkAudio, Display, TEXT("UAkSettings::CreateInitialAssetLibrary: Using existing Shared Asset Library at %s"), *Path.ToString());
			}
			else
			{
		...
		if (!AssetLibrary)
		{
			...
			UE_LOG(LogAkAudio, Display, TEXT("UAkSettings::CreateInitialAssetLibrary: Creating Shared Asset Library at %s"), *Path.ToString());
			...
			{
				UE_LOG(LogAkAudio, Error, TEXT("UAkSettings::CreateInitialAssetLibrary: Could not create Shared Asset Library at %s"), *Path.ToString());
				return;
			}

			auto* Filter = NewObject<UWwiseAssetLibraryFilterRemoveUniques>(AssetLibrary);
			if (UNLIKELY(!Filter))
			{
				UE_LOG(LogAkAudio, Error, TEXT("UAkSettings::CreateInitialAssetLibrary: Could not create Shared Filter for Asset Library at %s"), *Path.ToString());
				return;
			}
...
void UAkSettings::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	else if (MemberPropertyName == GET_MEMBER_NAME_CHECKED(UAkSettings, bPackageAsBulkData))
	{
		CreateInitialAssetLibrary();
	}
...
void UAkSettings::OnPostEngineInit()
{
	CreateInitialAssetLibrary();
}

bool UAkSettings::EnsurePostEngineInit()
{
		...
		CurrentPhase == ELoadingPhase::None || CurrentPhase < ELoadingPhase::PostDefault || !GEngine)
	{
		...
		{
			PostEngineInitDelegate = FCoreDelegates::OnPostEngineInit.AddUObject(this, &UAkSettings::OnPostEngineInit);
		}
		return false;
	}
