...
void UAkGameplayStatics::SetMaxReflectionPaths(int InMaxReflectionPaths)
{
	if (InMaxReflectionPaths < 0)
	{
		UE_LOG(LogAkAudio, Warning, TEXT("UAkGameplayStatics::SetMaxReflectionPaths: Invalid number of Reflection Paths (%d). Reflection path limit has been disabled."), InMaxReflectionPaths);
		InMaxReflectionPaths = 0;
	}

	auto* SpatialAudio = IWwiseSpatialAudioAPI::Get();
	if (UNLIKELY(!SpatialAudio)) return;
	SpatialAudio->SetMaxReflectionPaths(InMaxReflectionPaths);
}
